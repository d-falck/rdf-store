...
int _get_score(TriplePattern, std::unordered_set<Variable>);
    ...
    while (!unprocessed.empty()) {
        ...
        for (TriplePattern pattern : unprocessed) {
            ...
            bool condition = (score < best_score)
                && (vars.empty() || !utils::intersect<Variable>(vars, bound).empty());
        ...
        for (Variable var : utils::get_variables(best_pattern)) bound.insert(var);
...
int _get_score(TriplePattern pattern, std::unordered_set<Variable> bound) {
    auto is_bound = [=] (auto t) {return t.index() == 1 ||
                                         bound.count(std::get<Variable>(t));};
    auto [a,b,c] = pattern;
    if (is_bound(a)) {
